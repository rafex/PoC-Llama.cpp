...
ACTIVE_INSTALL_DIR := $(shell readlink $(INSTALL_CURRENT) 2>/dev/null || printf '%s' '$(INSTALL_DIR)')
WRAPPERS_DIR       := $(ACTIVE_INSTALL_DIR)/scripts
...
post-install-wrappers:
	...
	@sudo bash $(WRAPPER_SCRIPT) "$(ACTIVE_INSTALL_DIR)" "$(INSTALL_CURRENT)"
	@printf "$(GREEN)[OK]$(RESET)    Wrappers listos: $(WRAPPERS_DIR)\n"

## Ajusta permisos en el directorio de instalación
post-install-perms:
	$(call log_info,Ajustando permisos en $(ACTIVE_INSTALL_DIR) ...)
	@sudo chmod -R 755 $(ACTIVE_INSTALL_DIR)/bin 2>/dev/null || true
	@sudo chmod -R 755 $(ACTIVE_INSTALL_DIR)/scripts 2>/dev/null || true
